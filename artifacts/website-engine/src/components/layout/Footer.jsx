/**
 * Footer — config-driven, 3-column layout.
 *
 * Columns:
 *  1. Brand — logo / name, tagline, social links
 *  2. Quick links — from config.navigation
...
export function Footer({ config }) {
  const { meta, contact, navigation } = config ?? {};
    ...
    <footer className="bg-gray-900 text-gray-400">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid gap-10 md:grid-cols-3">
          ...
          <div>
            {meta?.logo_url ? (
              <img src={meta.logo_url} alt={meta.business_name} className="h-8 w-auto object-contain mb-3 brightness-0 invert" />
            ) : (
              <p
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              {contact?.phone && (
                <p>
                  <a href={`tel:${contact.phone}`} className="hover:text-[var(--color-primary)] transition-colors">
                    {contact.phone}
                  </a>
                </p>
              )}
                ...
                <div className="mt-3 pt-3 border-t border-gray-800 space-y-1">
                  {contact.hours.map((h, i) => (
                    <p key={i} className="text-gray-500">
                      <span className="text-gray-400">{h.day}:</span> {h.time}
                    </p>
                  ))}
                </div>
              )}
